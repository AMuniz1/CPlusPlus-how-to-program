#include <cstdint>
#include <string>
#include <cmath>

class DollarAmount{
    private:
        int64_t amount{0};

    public:
        // Construtor original (explicit previne conversões implícitas)
        explicit DollarAmount(int64_t value) : amount{value} { }

        // Construtor com dois parâmetros (dollars e pennies)
        DollarAmount(int64_t dollars, int64_t cents)
            : amount{dollars * 100 + cents} { }

        void add(DollarAmount right){
            amount += right.amount;
        }

        void subtract(DollarAmount right){
            amount -= right.amount;
        }

        void addInterest(int rate, int divisor) {
            DollarAmount interest{
                (amount * rate + divisor / 2) / divisor
            };
            add(interest);
        }

        // Nova função divide com arredondamento
        void divide(int divisor) {
            // Usa técnica similar a addInterest para arredondar
            // Adiciona divisor/2 antes de dividir para arredondar corretamente
            amount = (amount + divisor / 2) / divisor;
        }

        std::string toString() const{
            std::string dollars{std::to_string(amount/100)};
            std::string cents{std::to_string(std::abs(amount % 100))};
            return dollars + "." + (cents.size() == 1 ? "0" : "") + cents;
        }
};
